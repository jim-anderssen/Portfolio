...
def graph_dir(
    ...
    #directory = fr"{directory[3:]}"
    if directory[1:3] == ':/':
        directory = fr"{directory[3:]}"
    assert os.path.exists(directory), f'Invalid argument for "directory". {directory} is not in the current directory'
    ...
    for root, dirs, files in os.walk(os.path.normpath(f"./{directory}/")):
        ...
        ### eget
        try:
            dir_memory = convert(sum(os.path.getsize(os.path.join(root, f)) for f in files))
        except Exception as e:
            print(f'Error while getting size: {e}')
        ...
        if data and files:
            ...
            if not show_files and dirs:
                directory_data.extend(f" ({dir_memory})")
            directory_data.append("\l")
        ...
        ### eget
        path_to_file = "file:///" + os.path.abspath(parent_directory).replace("\\","/")
        tree.node(root, label="".join(directory_data), href=path_to_file)

        for dir in dirs:
            path = os.path.join(root, dir).replace(os.sep, "")
            
            ### egen ändring
            path_to_file = "file:///" + os.path.abspath(os.path.join(root, dir)).replace("\\","/")
            tree.node(path, label=dir,href=path_to_file)
        ...
        if files and show_files:
            ...
            for file in files:
                ...
                file_label = f"{file}\n({dir_memory})" if data else file
                file_href = "file:///" + os.path.abspath(os.path.join(root, file)).replace("\\", "/")
                tree.node(file_path, label=file_label, href=file_href)
                tree.edge(root, file_path)
